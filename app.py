    ...
    with right:
        ...
        if st.button("로그인", type="primary", use_container_width=True, key="login_btn"):
            if username and password:
                ...
                if user:
                    _tok = create_session(user["username"], remember=remember)
...
#  PDF generator (HTML)
# ══════════════════════════════════════════════════════════════════════
def gen_pdf_html(df, branches, year, month, exp_df=None, rev_df=None):
    ...
    for _, row in df[df.branch.isin(branches)].iterrows():
        ...
        for cat in CARD_CATS:
            amt = int(rev_by_cat.get(cat, 0))
            if amt > 0:
                card_detail_html += f'<tr class="sub"><td>&nbsp;&nbsp;{cat}</td><td class="amt">{fn(amt)} 원</td></tr>'
        if not card_detail_html:
            card_detail_html = '<tr class="sub"><td>&nbsp;&nbsp;내역 없음</td><td class="amt">—</td></tr>'

        cash_detail_html = ""
        for cat in CASH_CATS:
            amt = int(rev_by_cat.get(cat, 0))
            if amt > 0:
                cash_detail_html += f'<tr class="sub"><td>&nbsp;&nbsp;{cat}</td><td class="amt">{fn(amt)} 원</td></tr>'
        if not cash_detail_html:
            cash_detail_html = '<tr class="sub"><td>&nbsp;&nbsp;내역 없음</td><td class="amt">—</td></tr>'
        ...
        if not other_rows_html:
            other_rows_html = '<tr class="sub"><td>&nbsp;&nbsp;내역 없음</td><td class="amt">—</td></tr>'
